...
vec2 viewToScreen(vec3 viewPos) {
    ...
    return projCoords.xy * 0.5 + 0.5;
}

float get_linear_depth(vec2 uv) {
    float depth = getDepth(uv);
    return (2.0 * 1.0 * 5000.0) / (5000.0 + 1.0 - depth * (5000.0 - 1.0));
}
...
void main() {
    ...
    // vec3 lightDir = normalize(lightPosition - currentViewPos);
    ...
    for (int i = 0; i < MAX_STEPS; ++i) {
        ...
        float sceneDepth = get_linear_depth(uv);
        // FragColor = vec4(vec3(sceneDepth), 1.0);
        if(rayPos.z < 0.0){
            FragColor = vec4(vec3(0), 1.0);
            break;
        }
        // FragColor = vec4(vec3(rayPos.z), 1.0);
